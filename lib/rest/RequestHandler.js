                ...
                req.once("response", (resp) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.headers["x-ratelimit-limit"]) {
                            this.ratelimits[route].limit = Number(resp.headers["x-ratelimit-limit"]);
                        }
                        ...
                        this.ratelimits[route].remaining = resp.headers["x-ratelimit-remaining"] === undefined ? 1 : Number(resp.headers["x-ratelimit-remaining"]) || 0; // REVIEW Possible nullish coalescing?
                        ...
                        if(retryAfter >= 0) {
                            ...
                            let resetTime = Number(resp.headers["x-ratelimit-reset"]) * 1000;
                            if(route.endsWith("/reactions/:id") && (Number(resp.headers["x-ratelimit-reset"]) * 1000 - headerNow) === 1000) {
                                resetTime = now + 250;
                            }
