...
class Shard extends EventEmitter {
    ...
    */
    ...
    hardReset() {
        ...
        this.presence = {...this.client.presence}; // Fast copy
    ...
    identify() {
        ...
        const identify = {
            token: this._token,
            v: GATEWAY_VERSION,
            compress: Boolean(this.client.options.compress),
    ...
    onPacket(packet) {
        if(this.listeners("rawWS").length || this.client.listeners("rawWS").length) {
            /**
    ...
    requestGuildMembers(guildID, options) {
        ...
        if(!opts.query && !opts.user_ids && !(this.client.options.intents & Constants.Intents.guildMembers)) {
            throw new Error("Cannot request all members without guildMembers intent");
        }
        if(opts.presences && !(this.client.options.intents & Constants.Intents.guildPresences)) {
    ...
    sendStatusUpdate() {
        this.sendWS(GatewayOPCodes.PRESENCE_UPDATE, {
            ...
            afk: Boolean(this.presence.afk), // For push notifications
        ...
        if(this.ws?.readyState === WebSocket.OPEN) {
            ...
            const func = () => {
                if(++i >= waitFor && this.ws?.readyState === WebSocket.OPEN) {
                    const data = Erlpack?.pack({op: op, d: _data}) || JSON.stringify({op: op, d: _data});
                    this.ws.send(data);
                    if(_data.token) {
                        delete _data.token;
                    }
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                ...
                const oldState = {...member.voiceState};
            ...
            case "MESSAGE_DELETE_BULK": {
                ...
                * @prop {Array<Message> | Array<Object>} messages An array of (potentially partial) message objects. If a message is not cached, it will be an object with `id` and `channel` keys. If the channel is uncached, channel will be an object with an `id` key. If the uncached messages are from a guild, the messages will also contain a `guildID` key, and the channel will contain a `guild`, which if uncached, will only contain an `id` key. No other property is guaranteed.
                */
            ...
            case "MESSAGE_REACTION_ADD": {
                ...
                if(channel) {
                    ...
                    if(channel.guild && packet.d.member) {
                        // Updates the member cache with this member for future events.
                        packet.d.member.id = packet.d.user_id;
                ...
                * @prop {Message | Object} message The message object. If the message is not cached, this will be an object with `id` and `channel` keys. If the channel is not cached, channel will be an object with an `id` key. If the uncached message is from a guild, the message will also contain a `guildID` key, and the channel will contain a `guild`, which if uncached, will only contain an `id` key. No other property is guaranteed. If the channel is not cached, channel key will be an object with only an id. `guildID` will be present if the message was sent in a guild channel. No other property is guaranteed
            ...
            case "MESSAGE_REACTION_REMOVE": {
                ...
                if(message) {
                    ...
                    if(packet.d.guild_id) {
                        message.guildID = packet.d.guild_id;
                        message.channel.guild ||= this.client.guilds.get(packet.d.guild_id) || {id: packet.d.guild_id};
                    }
                }
                /**
                * Fired when someone removes a reaction from a message
                * @event Client#messageReactionRemove
                * @prop {Message | Object} message The message object. If the message is not cached, this will be an object with `id` and `channel` keys. If the channel is not cached, it will be an object with an `id` key. If the uncached message is from a guild, the message will also contain a `guildID` key, and the channel will contain a `guild`, which if uncached, will contain an `id` key. No other property is guaranteed
                * @prop {Object} emoji The reaction emoji object
                * @prop {Boolean?} emoji.animated Whether the emoji is animated or not
            ...
            case "GUILD_MEMBER_REMOVE": {
                ...
                * @prop {Guild} guild The guild
                * @prop {Member | Object} member The member. If the member is not cached, this will be an object with `id` and `user` key. No other property is guaranteed
                */
                ...
                if(guild.pendingVoiceStates?.length > 0) {
                    for(const voiceState of guild.pendingVoiceStates) {
                        ...
                        if(channel) {
                            ...
                            if(this.client.options.seedVoiceConnections && voiceState.id === this.client.user.id && !this.client.voiceConnections.get(channel.guild?.id || "call")) {
    ...
    _onWSClose(code, reason) {
        ...
        if(code) {
            ...
            switch(code) {
                case 4001: err = new Error("Gateway received invalid OP code"); break;
                case 4002: err = new Error("Gateway received invalid message"); break;
                case 4003: {
                ...
                case 4005: err = new Error("Already authenticated"); break;
                case 4006:
                case 4009: {
                ...
                case 4008: err = new Error("Gateway connection was ratelimited"); break;
                case 4010: {
                ...
                case 1006: err = new Error("Connection reset by peer"); break;
                case 1000: break;
                default: {
                    if(reason) {
