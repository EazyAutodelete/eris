...
class Shard extends EventEmitter {
    ...
    */
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "MESSAGE_UPDATE": {
                ...
                if(!channel.messages) console.log("MESSAGE_UPDATE but no cache: " + packet.d.id + " in " + channel.id + " (" + channel.type + ")");
                const message = channel.messages && channel.messages.get(packet.d.id);
                let oldMessage = null;
                if(message) {
                    oldMessage = {
            ...
            case "CHANNEL_CREATE": {
                ...
                if(packet.d.guild_id) {
                    ...
                    */
                    ...
                    // CHANNEL_CREATE type: " + JSON.stringify(packet, null, 2)));
                    break;
                }
                break;
            }
    ...
    _onWSClose(code, reason) {
        ...
        let err = !code || (code === 1000 || code === 1001) ? null : new Error(code + ": " + reason);
        let reconnect = "auto";
        if(code) {
