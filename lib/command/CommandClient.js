...
class CommandClient extends Client {
    ...
    */
            ...
            this.registerCommand("help", (msg, args) => {
                ...
                if(args.length > 0) {
                    ...
                    for(const label in this.commands) {
                        if(this.commands.hasOwnProperty(label) && this.commands[label]?.permissionCheck(msg) && !this.commands[label].hidden) {
    ...
    checkPrefix(msg) {
        const prefixes = this.guildPrefixes[msg.guildID] || this.commandOptions.prefix;
    ...
    */
    ...
    async onMessageReactionEvent(msg, emoji, reactor) {
        ...
        if(activeMessage?.command.reactionButtons) {
    ...
    */
    registerCommand(label, generator, options = {}) {
        ...
        if(this.commands[label] || (this.commands[lowercaseCommand]?.caseInsensitive)) {
        ...
        if(this.commands[command] || (this.commands[lowercaseCommand]?.caseInsensitive)) {
        ...
        options.caseInsensitive === true && (label = label.toLowerCase());
        ...
        options.aliases?.forEach((alias) => {
            ...
            if(this.commands[alias] || (this.commands[lowercaseCommand]?.caseInsensitive)) {
            ...
            options.caseInsensitive === true && (alias = alias.toLowerCase());
    ...
    */
    registerCommandAlias(alias, label) {
        ...
        caseInsensitiveLabel === true && (alias = alias.toLowerCase());
    ...
    */
    ...
    resolveCommand(label) {
        this.commandAliases[label] && (label = this.commandAliases[label]);
