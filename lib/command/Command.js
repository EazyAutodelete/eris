...
class Command {
    ...
    */
    constructor(label, generator, options, parentCommand) {
        ...
        this.caseInsensitive = Boolean(options.caseInsensitive);
        this.hooks = options.hooks || {};
        this.requirements = options.requirements || {};
        this.requirements.userIDs ||= [];
        this.requirements.permissions ||= {};
        this.deleteCommand = Boolean(options.deleteCommand);
        this.argsRequired = Boolean(options.argsRequired);
        this.guildOnly = Boolean(options.guildOnly);
        this.dmOnly = Boolean(options.dmOnly);
        ...
        this.restartCooldown = Boolean(options.restartCooldown);
        ...
        this.reactionButtons = options.reactionButtons?.map((button, index) => {
    ...
    cooldownExclusionCheck(msg) {
        return this.cooldownExclusions.channelIDs.includes(msg.channel.id) || this.cooldownExclusions.userIDs.includes(msg.author.id) || this.cooldownExclusions.guildIDs.includes(msg.guildID);
    }
    ...
    async executeCommand(msg, args) {
        if(this.hooks.postCheck) {
            ...
            if(response) {
                ...
                response.args && (msg = response.args);
            }
        }
