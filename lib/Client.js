    ...
    async getMessages(channelID, options = {}, before, after, around) {
        ...
        if(limit && limit > 100) {
            ...
            const get = async (_before, _after) => {
                ...
                logs = (_after
                    ? messages.map((message) => channel ? channel.messages.add(message) : new Message(message, this)).concat(logs)
                    : logs.concat(messages.map((message) => channel ? channel.messages.add(message) : new Message(message, this))));
