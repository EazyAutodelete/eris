    ...
    async getMessages(channelID, options = {}, before, after, around) {
        ...
        if(limit && limit > 100) {
            ...
            const get = async (_before, _after) => {
                ...
                logs = (_after
                    ? messages.map((message) => channel ? channel.messages.add(message, this) : new Message(message, this)).concat(logs)
                    : logs.concat(messages.map((message) => channel ? channel.messages.add(message, this) : new Message(message, this))));
        ...
        return messages.map((message) => {
            try {
                ...
                if(channel) {
                    channel.messages.add(message, this)
                    return channel.messages.get(message.id)
                }
