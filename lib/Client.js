...
class Client extends EventEmitter {
    ...
    */
    banGuildMember(guildID, userID, deleteMessageDays, reason) {
        if(!Number.isNaN(deleteMessageDays) && (deleteMessageDays < 0 || deleteMessageDays > 7)) {
    ...
    */
    deleteMessages(channelID, messageIDs, reason) {
        ...
        const oldestAllowedMessage = Date.now() - 1209600000;
        const invalidMessage = messageIDs.find((messageID) => Base.getCreatedAt(messageID) < oldestAllowedMessage);
        if(invalidMessage) {
    ...
    */
    executeWebhook(webhookID, token, options) {
        ...
        return this.requestHandler.request("POST", Endpoints.WEBHOOK_TOKEN(webhookID, token) + (qs ? "?" + qs : ""), Boolean(options.auth), {
    ...
    */
    getVoiceRegions(guildID) {
        return this.requestHandler.request("GET", guildID ? Endpoints.GUILD_VOICE_REGIONS(guildID) : Endpoints.VOICE_REGIONS, true);
    }
