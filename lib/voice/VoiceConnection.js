...
class VoiceConnection extends EventEmitter {
    constructor(id, options = {}) {
        ...
        this.opusOnly = Boolean(options.opusOnly);
    ...
    connect(data) {
        ...
        this.connectionTimeout = setTimeout(() => {
            ...
            this.connectionTimeout = null;
        }, this.shard.client?.options.connectionTimeout || 30000).unref();
        if(!data.endpoint) {
    ...
    */
    play(source, options = {}) {
        ...
        options.format ??= null;
        options.voiceDataTimeout = !Number.isNaN(options.voiceDataTimeout) ? options.voiceDataTimeout : 2000;
        options.inlineVolume = Boolean(options.inlineVolume);
        options.inputArgs ??= [];
        options.encoderArgs ??= [];

        options.samplingRate ??= this.samplingRate;
        options.frameDuration ??= this.frameDuration;
        options.frameSize ??= options.samplingRate * options.frameDuration / 1000;
        options.pcmSize ??= options.frameSize * 2 * this.channels;
    ...
    */
    ...
    registerReceiveEventHandler() {
        this.udpSocket.on("message", (msg) => {
            ...
            const hasExtension = Boolean(msg[0] & 0b10000);
    ...
    */
    ...
    setSpeaking(value, delay = 0) {
        this.speaking = typeof value === "boolean" ? Number(value) : value;
