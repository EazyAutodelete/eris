...
class Message extends Base {
  constructor(data, client) {
    ...
    if (this.channel && this.channel.hasOwnProperty("messages")) {
    ...
    switch (this.type) {
      ...
      case MessageTypes.GUILD_MEMBER_JOIN: {
        data.content = SystemJoinMessages[~~(this.createdAt % SystemJoinMessages.length)].replace(
          /%user%/g,
          this.author.mention
        );
        break;
  ...
  get channelMentions() {
    ...
    return (this._channelMentions = ((this.content && this.content.match(/<#[0-9]+>/g)) || []).map(mention =>
