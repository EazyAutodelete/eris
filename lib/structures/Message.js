...
class Message extends Base {
    constructor(data, client) {
        ...
        switch(this.type) {
            ...
            case MessageTypes.USER_PREMIUM_GUILD_SUBSCRIPTION_TIER_3: {
                data.content = `${this.author.mention} just boosted the server! ${this.channel.guild?.name || this._client.guilds.get(data.guild_id)?.name || data.guild_id} has achieved **Level ${this.type - 8}!**`;
                break;
            }
    ...
    get jumpLink() {
        return `${Endpoints.CLIENT_URL}${Endpoints.MESSAGE_LINK(this.channel.type === 1 ? "@me" : this.guildID, this.channel.id, this.id)}`;
    }
