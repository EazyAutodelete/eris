...
class MessageCollection extends Collection {
  constructor(...opts) {
    ...
    this._sweepInterval = setInterval(async () => {
      for (const [k, v] of this) {
        if (v && v.timestamp && Date.now() - v.timestamp > 14 * 24 * 60 * 60 * 1000) {
          this.delete(k);
        }
      }
    }, 30_000);
...
class ThreadChannel extends GuildChannel {
  constructor(data, client, messageLimit) {
    super(data, client);
    this.messages = new MessageCollection(Message, messageLimit == null ? client.options.messageLimit : messageLimit);
