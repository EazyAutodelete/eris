...
class GuildAuditLogEntry extends Base {
    ...
    get target() { // pay more, get less
        switch(true) {
            case this.actionType < 10: return this.guild; // Guild
            case this.actionType < 20: return this.guild?.channels.get(this.targetID); // Channel
            case this.actionType < 30: { // Member
            ...
            case this.actionType < 40: return this.guild?.roles.get(this.target); // Role
            case this.actionType < 50: { // Invite
            ...
            case this.actionType < 60: return null; // Webhook - Go get the webhook yourself
            case this.actionType < 70: return this.guild?.emojis.find((emoji) => emoji.id === this.targetID); // Emoji
            case this.actionType < 80: return this.guild?.shard.client.users.get(this.targetID); // Message
            case this.actionType < 83: return null; // Integrations
            case this.actionType < 90: return this.guild?.stageInstances.get(this.targetID); // Stage Instances
            case this.actionType < 100: return this.guild?.stickers.find((sticker) => sticker.id === this.targetID); // Sticker
            case this.actionType < 110: return null; // Guild Events, #1275
            case this.actionType < 120: return this.guild?.threads.get(this.targetID); // Threads
            default: throw new Error("Unrecognized action type: " + this.actionType);
        }
    }
